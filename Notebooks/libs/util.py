...
class MaskGenerator():
    ...
    def _generate_mask(self):
        ...
        """Only applicable to 192 x 192 images"""
        #img = np.zeros((self.height, self.width, self.channels), np.uint8)
        
        # Generates random missing (days) 
        idx_start = randint(1, self.height*self.width)
        missing_rate = round(random()/2, 2)
        img_new = img.copy()
        mask_missing = np.random.permutation(self.height*self.width).reshape(self.height,self.width,self.channels)/self.height/self.width

        img_new[mask_missing<missing_rate] = 1
            
        img_new = img_new.reshape(self.height,self.width,self.channels)  
        
        # Generates continuous missing 
        idx_start = randint(1, self.height*self.width)
        missing_gaps = randint(1, self.height*self.width/2)
        img_new = img_new.flatten()
                
        if idx_start+missing_gaps<=self.height*self.width:
            img_new[idx_start:idx_start+missing_gaps] = 1
        else:
            img_new[-(idx_start+missing_gaps):-idx_start] = 1        
        img_new = img_new.reshape(self.height,self.width,self.channels)
